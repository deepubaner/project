...
function displayCart() {
     ...
     if (cart.size === 0) {
          return cartContainer.innerHTML = ' <img src = "illustration-empty-cart.svg" alt = "illustration-empty-cart"/><p>Your added items would appear here</p>'
     }
...
document.querySelector("#confirm-btn").addEventListener("click", () => {
    const cartData = Array.from(cart.values()).map(({ product, quantity }) => ({
        name: product.name,
        price: product.price,
        quantity: quantity
    }));
